...
class PlannerCarryOverTest {
  @Test
  fun incompleteOlderTasksMoveToTargetDate() {
  ...
  @Test
  fun completedAndFutureTasksAreNotCarried() {
    val tasks = listOf(
      PlannerTaskEntity(id = "done", title = "Done", scheduledDate = "2026-09-18", isCompleted = true),
      PlannerTaskEntity(id = "future", title = "Future", scheduledDate = "2026-09-20")
    )
  ...
  @Test
  fun originalCarryDateIsPreservedAcrossMultipleDays() {
    val task = PlannerTaskEntity(
      id = "1",
      title = "Revise OS",
      scheduledDate = "2026-09-18",
      carriedFromDate = "2026-09-17"
    )

    val carried = carryIncompleteTasks(listOf(task), "2026-09-19")

    assertEquals("2026-09-17", carried.first().carriedFromDate)
  }
}
