...
fun List<QuestionAttemptEntity>.revisionReasons(): Set<RevisionReason> {
  ...
  val hasSlowAnswer = any { attempt -> attempt.timeTakenMs >= RevisionConstants.slowAnswerMs }
  return buildSet {
    ...
    if (correctCount * 100 / attemptCount < LOW_ACCURACY_PERCENT) add(RevisionReason.LOW_ACCURACY)
  }
}

fun Set<RevisionReason>.summary(): String = if (isEmpty()) "No revision reason recorded" else joinToString(" • ") { it.label }
