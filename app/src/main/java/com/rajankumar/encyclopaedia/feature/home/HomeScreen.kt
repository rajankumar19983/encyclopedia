...
fun HomeScreen() {
  Column(
    ...
    LazyVerticalGrid(
      ...
      items(
        listOf(
          Stat("Topics", "0", "Build your knowledge tree", Icons.Default.AutoStories, 0f),
          Stat("Questions", "0", "Your question bank is ready", Icons.Default.Quiz, 0f),
          Stat("Day Streak", "0", "Your streak starts today", Icons.Default.LocalFireDepartment, 0f),
          Stat("Overall Progress", "0%", "Practice to build progress", Icons.Default.CheckCircle, 0f)
        )
      ) { stat -> StatCard(stat) }

      item(span = { androidx.compose.foundation.lazy.grid.GridItemSpan(2.coerceAtMost(maxLineSpan)) }) {
        TodayPlanCard()
      }
...
private fun TodayPlanCard() {
  Card(
    ...
    Column(
      ...
      Text(
        "Complete the plan first, then explore anything you want.",
        color = MaterialTheme.colorScheme.onSurfaceVariant
      )
    }
  }
