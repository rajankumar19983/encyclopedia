...
fun PerformanceScreen() {
  ...
  val weeklySummary = remember(data.dailyActivity) { data.dailyActivity.weeklySummary() }
  val weeklyPace = remember(weeklySummary) { weeklySummary.studyPace() }
  val weeklyFeedback = remember(weeklySummary) { weeklySummary.feedback() }
  ...
  LazyColumn(Modifier.fillMaxSize().padding(28.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
    ...
    item { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) { PerformancePeriod.entries.forEach { option -> FilterChip(selected = dashboardState.period == option, onClick = { dashboardState = dashboardState.copy(period = option) }, label = { Text(option.label) }) } } }
    item { Text(data.narrative(), color = MaterialTheme.colorScheme.onSurfaceVariant) }
    item { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) { summary.stats().forEach { stat -> Card(Modifier.weight(1f)) { Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) { Text(stat.label, style = MaterialTheme.typography.labelLarge); Text(stat.value, style = MaterialTheme.typography.headlineSmall); Text(stat.detail, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } } } } }
    ...
    item {
      Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
          ...
          Text(weeklyFeedback.headline, style = MaterialTheme.typography.titleSmall)
          Text("${weeklySummary.activeDays} active days • ${weeklySummary.attempts} attempts • ${weeklySummary.accuracyPercent}% accuracy")
          Text("${weeklyPace.attemptsPerActiveDay} attempts per active day • ${formatDurationMs(weeklyPace.averageTimePerActiveDayMs)} average study time", style = MaterialTheme.typography.bodySmall)
          Text(weeklyFeedback.detail, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
      }
    }
    ...
    item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) { Text("Topic coverage", style = MaterialTheme.typography.titleMedium); LinearProgressIndicator(progress = { topicCoverage.coveragePercent / 100f }, modifier = Modifier.fillMaxWidth()); Text("${topicCoverage.practisedTopicCount} of ${topicCoverage.topicCount} topics practised • ${topicCoverage.coveragePercent}%"); Text("${topicCoverage.strongTopicCount} strong • ${topicCoverage.weakTopicCount} weak • ${topicCoverage.unpractisedTopicCount} unpractised", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant); Text(topicInsight.headline, style = MaterialTheme.typography.titleSmall); Text(topicInsight.detail, color = MaterialTheme.colorScheme.onSurfaceVariant) } } }
