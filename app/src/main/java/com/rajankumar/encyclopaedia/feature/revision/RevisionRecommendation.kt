...
fun List<RevisionItem>.revisionRecommendation(): String {
  ...
  return when {
    stats.total == 0 -> "No revision queue yet. Practice MCQs to identify material that needs another look."
    stats.urgent > 0 -> "Start with ${stats.urgent} urgent question${if (stats.urgent == 1) "" else "s"} before lower-priority review."
