...
fun NotebookScreen() {
  ...
  LazyColumn(modifier = Modifier.fillMaxSize().padding(28.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
      ...
      Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
          ...
          Button(enabled = title.isNotBlank(), onClick = {
            val pageTitle = title.trim(); title = ""
            scope.launch(Dispatchers.IO) { val pageId = UUID.randomUUID().toString(); dao.upsertNotebookPage(NotebookPageEntity(id = pageId, title = pageTitle)); dao.upsertNotebookLayer(NotebookLayerEntity(id = UUID.randomUUID().toString(), pageId = pageId, name = "Writing")) }
          }) { Text("Create page") }
        }
      }
    }
    if (pages.isEmpty()) item { Text("No notebook pages yet. Create one above to start writing.") }
      ...
      Card(Modifier.fillMaxWidth().clickable { selectedPage = page }) {
        Row(Modifier.fillMaxWidth().padding(16.dp), horizontalArrangement = Arrangement.SpaceBetween) {
          Column(Modifier.weight(1f)) { Text(page.title, style = MaterialTheme.typography.titleMedium); Text(page.background.lowercase().replaceFirstChar { it.uppercase() } + " page", color = MaterialTheme.colorScheme.onSurfaceVariant) }
...
private fun NotebookPageCanvas(page: NotebookPageEntity, onBack: () -> Unit) {
  ...
  val stored by (activeLayer?.let { dao.observeNotebookStrokes(it.id) } ?: kotlinx.coroutines.flow.flowOf(emptyList())).collectAsStateWithLifecycle(emptyList())
  var palmRejection by remember { mutableStateOf(true) }
  val canvasStrokes = remember(stored) { stored.mapNotNull(::decodeStroke) }

  Column(Modifier.fillMaxSize().padding(20.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
      TextButton(onClick = onBack) { Text("← Pages") }
      Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { Text("Palm rejection"); Switch(checked = palmRejection, onCheckedChange = { palmRejection = it }) }
    }
    ...
    Card(Modifier.fillMaxWidth().weight(1f)) {
      if (activeLayer == null) Text("No writable layer.", Modifier.padding(16.dp))
      else NotebookCanvas(
        strokes = canvasStrokes,
        palmRejection = palmRejection,
        onStrokeFinished = { stroke ->
          scope.launch(Dispatchers.IO) { dao.upsertNotebookStroke(NotebookStrokeEntity(id = UUID.randomUUID().toString(), layerId = activeLayer.id, pointsJson = encodePoints(stroke), width = stroke.width)) }
        },
...
private fun decodeStroke(stroke: NotebookStrokeEntity): CanvasStroke? = runCatching {
  ...
  val points = (0 until array.length()).map { index -> val point = array.getJSONObject(index); androidx.compose.ui.geometry.Offset(point.getDouble("x").toFloat(), point.getDouble("y").toFloat()) }
  CanvasStroke(points, stroke.width)
}.getOrNull()
