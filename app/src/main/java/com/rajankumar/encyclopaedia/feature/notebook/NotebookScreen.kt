...
private fun NotebookPageCanvas(page:NotebookPageEntity,onBack:()->Unit){
  ...
  var palm by remember{mutableStateOf(prefs.palmRejection)};var tool by remember{mutableStateOf(NotebookTool.PEN)};var penWidth by remember{mutableStateOf(prefs.penWidth)};var penColor by remember{mutableStateOf(prefs.penColorArgb)};var newLayer by remember{mutableStateOf("")};var deleteLayer by remember{mutableStateOf<NotebookLayerEntity?>(null)}
  ...
  Column(Modifier.fillMaxSize().padding(20.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
    Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){TextButton(onClick=onBack){Text("← Pages")};Row{Text("Palm rejection");Switch(palm,{palm=it;prefs.palmRejection=it})}}
    ...
    Row(horizontalArrangement=Arrangement.spacedBy(6.dp)){FilterChip(tool==NotebookTool.PEN,{tool=NotebookTool.PEN},label={Text("Pen")});FilterChip(tool==NotebookTool.ERASER,{tool=NotebookTool.ERASER},label={Text("Eraser")});listOf(2.5f,4f,7f).forEach{w->FilterChip(penWidth==w,{penWidth=w;prefs.penWidth=w},label={Text(if(w<3)"Fine" else if(w<6)"Medium" else "Bold")})};OutlinedButton(enabled=undo.isNotEmpty(),onClick={val a=undo.removeLastOrNull()?:return@OutlinedButton;redo.add(a);scope.launch(Dispatchers.IO){if(a.added)dao.deleteNotebookStroke(a.stroke.id)else dao.upsertNotebookStroke(a.stroke)}}){Text("Undo")};OutlinedButton(enabled=redo.isNotEmpty(),onClick={val a=redo.removeLastOrNull()?:return@OutlinedButton;undo.add(a);scope.launch(Dispatchers.IO){if(a.added)dao.upsertNotebookStroke(a.stroke)else dao.deleteNotebookStroke(a.stroke.id)}}){Text("Redo")}}
    Row(horizontalArrangement=Arrangement.spacedBy(6.dp)){Text("Ink:");penColors.forEachIndexed{i,c->FilterChip(selected=penColor==c,onClick={penColor=c;prefs.penColorArgb=c},label={Text(listOf("Black","Blue","Red","Green")[i])})}}
